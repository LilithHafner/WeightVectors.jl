...
function _set_from_zero!(m::Memory, v::Float64, i::Int)
    ...
    if m[5] == 0 # if we were empty, set global shift (m[3]) so that m[5] will become ~2^40.
        ...
        if Base.top_set_bit(significand_sum)+shift > 64
            ...
            @noinline set_global_shift_decrease!(m, m3) # TODO for perf: special case all call sites to this function to take advantage of known shift direction and/or magnitude
        ...
        if o
            ...
            m3 = m[3]-0x10
            @noinline set_global_shift_decrease!(m, m3, m5) # TODO for perf: special case all call sites to this function to take advantage of known shift direction and/or magnitude

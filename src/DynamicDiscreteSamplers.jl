...
function _set_from_zero!(m::Memory, v::Float64, i::Int)
    ...
    group_length_index = _convert(Int, 6246 + 2exponent)
...
get_alloced_indices(exponent::UInt64) = _convert(Int, 10479 + exponent >> 3), exponent << 3 & 0x38
get_level_weights_nonzero_indices(exponent::UInt64) = _convert(Int, 10443 + exponent >> 6), exponent & 0x3f
...
function _set_to_zero!(m::Memory, i::Int)
    ...
    m[_convert(Int, shifted_element) + 10735] = _convert(UInt64, pos) << 12 + exponent
    m[j] = 0
...
function initialize_empty(len::Integer)
    ...
    m[4:10735+len] .= 0 # metadata and edit map need to be zeroed but the bulk does not
    m[1] = len
    m[2] = 4
    # no need to set m[3]
    m[10478] = 10736+len
...
allocated_memory(length::Integer) = 10735 + 7*length # TODO for perf: consider giving some extra constant factor allocation to avoid repeated compaction at small sizes
length_from_memory(allocated_memory::Integer) = Int((allocated_memory-10735)/7)
...
function _resize!(w::ResizableWeights, len::Integer)
    ...
    m2[1] = len
    if len > old_len # grow
        unsafe_copyto!(m2, 2, m, 2, old_len + 10735)
        m2[old_len + 10736:len + 10735] .= 0
    else # shrink
        unsafe_copyto!(m2, 2, m, 2, len + 10735)
    end
...
function compact!(dst::Memory{UInt64}, src::Memory{UInt64})
    dst_i = length_from_memory(length(dst)) + 10736
    src_i = length_from_memory(length(src)) + 10736
    next_free_space = src[10478]

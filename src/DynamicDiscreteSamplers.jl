...
function Base.push!(wbs::WeightBasedSampler, index, weight)
    index ∈ eachindex(wbs.w) && !(0 in wbs.w[index]) && throw(ArgumentError("Element $index is already present"))
...
function Base.append!(wbs::WeightBasedSampler, inds::AbstractVector, weights::AbstractVector)
    ...
    for (i,w) in zip(inds, weights)
        i ∈ eachindex(wbs.w) && !(0 in wbs.w[i]) && throw(ArgumentError("Element $i is already present"))
        wbs.w[i] = w
    end
    wbs
end
function Base.delete!(wbs::WeightBasedSampler, index)
    index ∈ eachindex(wbs.w) && !(0 in wbs.w[index]) || throw(ArgumentError("Element $index is not present"))
    wbs.w[index] = 0
    wbs
end
Base.in(index::Int, wbs::WeightBasedSampler) = wbs.w[index+10491] == 0

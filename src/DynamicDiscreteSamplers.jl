...
# 5..2102                level weights::[UInt64 2047] # earlier is lower. first is exponent 0x001, last is exponent 0x7fe. Subnormal are not supported (TODO).
# 2103..6246             significand_sums::[UInt128 2047] # sum of significands (the maximum significand contributes 0xfffffffffffff800)
# 6247..10442            level location info::[NamedTuple{pos::Int, length::Int} 2046] indexes into sub_weights, pos is absolute into m.
# 10443..10475           level_weights_nonzero::[Bool 2047] # map of which levels have nonzero weight (used to bump m2 efficiently when a level is zeroed out)
# 2 unused bits

# gc info:
# 10478                  next_free_space::Int (used to re-allocate)
# 16 unused bits
# 10479..10735           level allocated length::[UInt8 2047] (2^(x-1) is implied)

# 10736+len..10735+len   edit_map (maps index to current location in sub_weights)::[(pos<<11 + exponent)::UInt64] (zero means zero; fixed location, always at the start. Force full realloc when it OOMs. (len refers to allocated length, not m[1])

# 10736+2len..10735+7len sub_weights (woven with targets)::[[significand::UInt64, target::Int}]]. allocated_len == length_from_memory(length(m)) (len refers to allocated length, not m[1])
...
# the range 0x8000000000000001 to 0x80000000000007ff indicates that this is an empty but non-abandoned group with exponent target-0x8000000000000000
...
function _set_from_zero!(m::Memory, v::Float64, i::Int)
    ...
    if m[4] == 0 # if we were empty, set global shift (m[3]) so that m[4] will become ~2^40.
        ...
        exponent != 1 && @assert Base.top_set_bit(weight-1) == 40 # TODO for perf: delete
    ...
    group_length_index = _convert(Int, 3 + 3*2047 + 2exponent)
...
function set_global_shift_increase!(m::Memory, m2, m3::UInt64, m4) # Increase shift, on deletion of elements
    ...
    @inbounds for i in r0:min(r1, -61-signed(m3))
        significand_sum_lo = m[_convert(Int, 2i+2042)]
        significand_sum_hi = m[_convert(Int, 2i+2043)]
...
get_alloced_indices(exponent::UInt64) = _convert(Int, 10274 + exponent >> 3), exponent << 3 & 0x38
get_level_weights_nonzero_indices(exponent::UInt64) = _convert(Int, 10239 + exponent >> 6), exponent & 0x3f
...
function _set_to_zero!(m::Memory, i::Int)
    ...
    m[_convert(Int, shifted_element) + 10529] = _convert(UInt64, pos) << 11 + exponent
    m[j] = 0
...
function initialize_empty(len::Integer)
    ...
    m[4:10529+len] .= 0 # metadata and edit map need to be zeroed but the bulk does not
    m[1] = len
    m[2] = 4
    # no need to set m[3]
    m[10273] = 10530+len
...
allocated_memory(length::Integer) = 10529 + 7*length # TODO for perf: consider giving some extra constant factor allocation to avoid repeated compaction at small sizes
length_from_memory(allocated_memory::Integer) = Int((allocated_memory-10529)/7)
...
function _resize!(w::ResizableWeights, len::Integer)
    ...
    m2[1] = len
    if len > old_len # grow
        unsafe_copyto!(m2, 2, m, 2, old_len + 10529)
        m2[old_len + 10530:len + 10529] .= 0
    else # shrink
        unsafe_copyto!(m2, 2, m, 2, len + 10529)
    end
...
function compact!(dst::Memory{UInt64}, src::Memory{UInt64})
    dst_i = length_from_memory(length(dst)) + 10530
    src_i = length_from_memory(length(src)) + 10530
    next_free_space = src[10273]
    ...
    while src_i < next_free_space
        ...
        while target < 0
            if unsigned(target) < 0xc000000000000000 # empty non-abandoned group; let's clean it up
                @assert 0x8000000000000001 <= unsigned(target) <= 0x80000000000007ff
